...
resource "azurerm_network_interface" "fortigate_external_network_interface" {
  ...
  ip_configuration {
    ...
    private_ip_address            = cidrhost(var.external-subnet_prefix, 4)
    subnet_id                     = azurerm_subnet.external_subnet.id
    public_ip_address_id          = azurerm_public_ip.mgmt_public_ip.id #checkov:skip=CKV_AZURE_119:Fortigate gets a public IP
  }
  ip_configuration {
    name                          = "VIP-external-ipconfig"
    private_ip_address_allocation = "Static"
    private_ip_address            = cidrhost(var.external-subnet_prefix, 100)
    subnet_id                     = azurerm_subnet.external_subnet.id
    public_ip_address_id          = azurerm_public_ip.vip_public_ip.id #checkov:skip=CKV_AZURE_119:Fortigate gets a public IP
  }
}
...
resource "azurerm_network_interface" "fortigate_internal_network_interface" {
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Static"
    private_ip_address            = cidrhost(var.internal-subnet_prefix, 4)
