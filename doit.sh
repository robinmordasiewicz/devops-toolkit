...
#google-chrome --headless=new --screenshot="docs/img/github-fork-repo.png" --hide-scrollbars --window-size=1620,1080 "https://github.com/robinmordasiewicz/fortinet-devops/fork"

#sgpt --code "write a markdown page to let people know how to sign up for github copilot" > docs/github-copilot.md
#sgpt --code "writes markdown page to describe a demo where a user forks a github repository robinmordasiewicz/fortinet-devops branch named feature1 is created for a user to make their commits. The style guide conventional commits is used as a reference in creating commits. The use will use opencommit command from a zsh shell. In the markdown guide, create steps as an unordered list. Also in the markdown file show the bash commands to execute each step. Give each section a short title. As a pre-requisite instruction, have users install opencommit from github and use the cli command oco to stage, commit and push changes. Highlight a step that demonstrates following style guides for commits such as the conventional commits style guide. The user will use the opencommit command from a zsh shell. In the markdown guide, create steps as an unordered list. Also in the markdown file show the bash commands to execute each step. Give each section a short title." > docs/demo.md
